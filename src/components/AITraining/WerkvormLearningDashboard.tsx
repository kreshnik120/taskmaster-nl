...
import { CheckCircle2, XCircle, Play, RefreshCw, Briefcase, TrendingUp, Clock, AlertTriangle } from "lucide-react";
...
export function WerkvormLearningDashboard() {
  const queryClient = useQueryClient();
  const [isTriggering, setIsTriggering] = useState(false);

  // Fetch pending suggestions
  const { data: suggestions, isLoading: loadingSuggestions } = useQuery({
    queryKey: ["werkvorm-suggestions"],
    queryFn: async () => {
  ...
  const { data: functionLogs } = useQuery({
    queryKey: ["werkvorm-function-logs"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("function_call_logs")
        .select("*")
        .in("function_name", ["learn-werkvorm-patterns", "apply-learned-werkvorm"])
        .order("created_at", { ascending: false })
        .limit(10);
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["werkvorm-suggestions"] });
  ...
  const handleManualTrigger = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke("learn-werkvorm-patterns");
      if (error) throw error;
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Werkvorm Learning Dashboard
            </CardTitle>
            <CardDescription>
              AI-gestuurde normalisatie van werkvormen uit sollicitaties
            </CardDescription>
          </div>
                  ...
                  <TableRow key={suggestion.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">
                        {suggestion.value?.suggested_mapping || suggestion.value?.target || "—"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge
        ...
        <div>
          ...
          {loadingLearned ? (
            ...
            <p className="text-muted-foreground text-sm">
              Nog geen geleerde mappings. Het systeem leert automatisch dagelijks om 04:30 UTC.
            </p>
          ) : (
