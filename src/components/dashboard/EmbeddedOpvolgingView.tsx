...
/**
 * Embedded Opvolging View for Dashboard tab
...
export default function EmbeddedOpvolgingView() {
  ...
  const handleCompleteTask = async (taskId: string, taskTitle: string) => {
    ...
    try {
      ...
      console.error("[EmbeddedOpvolging] Error completing task:", error);
  ...
  const undoComplete = async (taskId: string) => {
    try {
      ...
      console.error('[EmbeddedOpvolging] Error undoing complete:', error);
