...
export function VacancyEditDialog({
  ...
  const [formData, setFormData] = useState({
    ...
    deadline: "",
    beschrijving: "",
    gewenste_sector_ervaring: [] as string[],
    gewenste_doelgroep_ervaring: [] as string[],
    vereiste_certificaten: [] as string[],
  });
  const [newCertificaat, setNewCertificaat] = useState("");

  // Initialize form data when vacancy changes
  useEffect(() => {
    if (vacancy && open) {
      setFormData({
        titel: vacancy.titel || "",
        functie_niveau: vacancy.functie_niveau || "",
        uren_per_week: vacancy.uren_per_week || 32,
        urgentie: vacancy.urgentie || "normaal",
        start_datum: vacancy.start_datum || "",
        deadline: vacancy.deadline || "",
        beschrijving: vacancy.beschrijving || "",
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Vacature bewerken</DialogTitle>
        </DialogHeader>
