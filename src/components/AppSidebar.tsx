        ...
        <SidebarMenu>
          ...
          return <SidebarMenuItem key={item.title}>
                <SidebarMenuButton asChild>
                  <NavLink to={item.url} className="text-slate-950">
                    <item.icon className="h-4 w-4 flex-shrink-0" />
                    <span className="flex-1">{item.title}</span>
                    
                    {badgeCount !== undefined && badgeCount > 0 && <Badge variant={item.badge === 'validationCount' ? 'destructive' : 'default'} className="ml-auto shrink-0">
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarGroupContent className="space-y-1">
            {menuGroups.map(group => <CollapsibleGroup key={group.label} group={group} activeTaskCount={activeTaskCount} validationCount={validationCount} canEdit={canEdit()} isAdmin={isAdmin()} />)}
          </SidebarGroupContent>
        </SidebarGroup>
      </SidebarContent>
      <SidebarFooter>
        <UserProfileCard user={currentUser} role={role} onLogout={handleLogout} onSwitch={handleSwitchAccount} />
      </SidebarFooter>
    </Sidebar>;
}