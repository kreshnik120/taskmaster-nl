...
interface MeetingAttachmentPreviewModalProps {
...
export function MeetingAttachmentPreviewModal({
  attachment,
  open,
  onOpenChange,
  onDownload
}: MeetingAttachmentPreviewModalProps) {
  const [loading, setLoading] = useState(true);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [zoom, setZoom] = useState(1);

  const fileCategory = attachment ? getFileCategory(attachment.file_name) : 'other';
  const isPreviewable = attachment ? canPreview(attachment.file_name) : false;
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] p-0 overflow-hidden">
        ...
        <DialogHeader className="p-4 border-b bg-muted/30">
          ...
          <DialogDescription className="sr-only">
            Bekijk bestand: {attachment.file_name}
          </DialogDescription>
        </DialogHeader>
