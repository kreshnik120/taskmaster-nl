/**
 * Learn Werkvorm Patterns
 * 
 * AI Learning Edge Function that:
 * 1. Aggregates unknown werkvorm values from system_events
 ...
 * 
 * Schedule: Daily at 04:30 UTC via master-scheduler
 * Enterprise Version: 1.0.0
 */
import { corsHeaders, handleCors, createAdminClient, jsonResponse, errorResponse } from '../_shared/core.ts';
import { WERKVORM_MAP, VALID_WERKVORMEN, ORG_IDS } from '../_shared/healthcare-mappings.ts';
...
/**
 * Find the closest valid werkvorm match using Levenshtein distance
 */
function findClosestMatch(input: string): { match: string; similarity: number } {
  const normalized = input.toLowerCase().trim();
  let bestMatch: string = VALID_WERKVORMEN[0];
  let bestDistance = Infinity;
  
  // Check against all valid values
  for (const valid of VALID_WERKVORMEN) {
  ...
  for (const [key, value] of Object.entries(WERKVORM_MAP)) {
...
Deno.serve(async (req) => {
  ...
  console.log(`🧠 [Learn WV Patterns v${VERSION}] Starting pattern learning cycle...`);

  try {
    const supabase = createAdminClient();
    
    // =========================================================================
    // STEP 1: Aggregate unprocessed unknown werkvorm events
    // =========================================================================
    const { data: events, error: eventsError } = await supabase
      .from('system_events')
      .select('id, event_data, org_id, created_at')
      .eq('event_type', 'werkvorm_unknown')
    ...
    console.log(`📊 [Learn WV] Found ${events?.length || 0} unprocessed events`);
    ...
    console.log(`📈 [Learn WV] Grouped into ${valueGroups.size} unique values`);
    ...
    for (const [rawValue, group] of valueGroups) {
      ...
      if (group.count < MIN_OCCURRENCES) {
        skipped++;
        console.log(`⏭️ [Learn WV] Skipped "${rawValue}" (${group.count} < ${MIN_OCCURRENCES} occurrences)`);
        continue;
      }
      
      // Generate unique key for this suggestion
      const suggestionKey = `wv_suggestion_${rawValue.replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')}`;
      
      // Check if suggestion already exists
      const { data: existing } = await supabase
        .from('ai_knowledge_base')
        .select('id, value, occurrence_count')
        .eq('category', 'werkvorm_mapping_suggestion')
      ...
      if (existing) {
        ...
        updated++;
        console.log(`🔄 [Learn WV] Updated "${rawValue}" (now ${newCount} occurrences)`);
      } else {
        ...
        const { error: insertError } = await supabase.from('ai_knowledge_base').insert({
          category: 'werkvorm_mapping_suggestion',
          key: suggestionKey,
          value: {
            raw_value: rawValue,
        ...
        if (insertError) {
          console.error(`❌ [Learn WV] Failed to create suggestion for "${rawValue}":`, insertError);
        } else {
          created++;
          console.log(`✨ [Learn WV] Created suggestion: "${rawValue}" → "${match}" (confidence: ${(confidence * 100).toFixed(1)}%)`);
        }
      }
    }
    ...
    if (processedEventIds.length > 0) {
      ...
      if (updateError) {
        console.error('⚠️ [Learn WV] Failed to mark events as processed:', updateError);
      } else {
        console.log(`✅ [Learn WV] Marked ${processedEventIds.length} events as processed`);
      }
    }
    ...
    await supabase.from('function_call_logs').insert({
      function_name: 'learn-werkvorm-patterns',
    ...
    console.log(`🏁 [Learn WV v${VERSION}] Complete in ${duration}ms: ${created} created, ${updated} updated, ${skipped} skipped`);
    ...
    const duration = Date.now() - startTime;
    console.error(`❌ [Learn WV v${VERSION}] Error:`, error);
    ...
    try {
      ...
      console.error('[Learn WV] Failed to log error:', logError);
    }
