/**
 * Apply Learned Werkvorm
 * 
 * Promotes high-confidence werkvorm suggestions to learned mappings.
 * 
 ...
 * 
 * Schedule: Daily at 04:45 UTC via master-scheduler
...
Deno.serve(async (req) => {
  ...
  console.log(`🚀 [Apply WV v${VERSION}] Starting auto-promotion cycle...`);
  ...
  try {
    ...
    console.log(`📊 [Apply WV] Found ${suggestions?.length || 0} eligible suggestions for promotion`);
    ...
    for (const suggestion of suggestions || []) {
      ...
      if (!rawValue || !targetMapping) {
        console.warn(`⚠️ [Apply WV] Skipping invalid suggestion ${suggestion.id}`);
        failed++;
        continue;
      }
      
      // Generate key for learned mapping
      const learnedKey = `wv_learned_${rawValue.replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')}`;
      ...
      if (existingLearned) {
        console.log(`⏭️ [Apply WV] "${rawValue}" already learned, skipping`);
      ...
      const { error: insertError } = await supabase.from('ai_knowledge_base').insert({
        category: 'werkvorm_mapping_learned',
        key: learnedKey,
        value: {
          raw_value: rawValue,
      ...
      if (insertError) {
        console.error(`❌ [Apply WV] Failed to create learned mapping for "${rawValue}":`, insertError);
        failed++;
        continue;
      }
      ...
      promoted++;
      console.log(`✅ [Apply WV] Promoted: "${rawValue}" → "${targetMapping}" (confidence: ${(suggestion.confidence_score * 100).toFixed(1)}%)`);
    }
    ...
    if (reviewCandidates && reviewCandidates.length > 0) {
      ...
      if (!flagError) {
        ...
        console.log(`🔍 [Apply WV] Flagged ${flaggedForReview} suggestions for manual review`);
      }
    }
    ...
    await supabase.from('function_call_logs').insert({
      function_name: 'apply-learned-werkvorm',
    ...
    console.log(`🏁 [Apply WV v${VERSION}] Complete in ${duration}ms: ${promoted} promoted, ${failed} failed, ${flaggedForReview} flagged`);
    ...
    const duration = Date.now() - startTime;
    console.error(`❌ [Apply WV v${VERSION}] Error:`, error);
    ...
    try {
      ...
      console.error('[Apply WV] Failed to log error:', logError);
    }
