...
// Get access token - use direct token if available, otherwise throw error
function getAccessToken(): string {
  const accessToken = Deno.env.get("TRANSIP_ACCESS_TOKEN");
  
  if (!accessToken) {
    throw new Error("TRANSIP_ACCESS_TOKEN secret not configured");
  }

  console.log("[TransIP] Using direct access token");
  return accessToken;
}
