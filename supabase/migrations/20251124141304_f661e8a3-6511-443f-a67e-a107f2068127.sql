-- Fix log_task_events trigger with semantically correct user tracking and edge case handling
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.completed_at IS NULL AND NEW.completed_at IS NOT NULL THEN
    ...
    VALUES (
      NEW.org_id,
      COALESCE(auth.uid(), NEW.assignee_id, NEW.reporter_id), -- ✅ FIXED: Prioritize auth.uid(), fallback to assignee/reporter
      'task_completed',
      'task',
      NEW.id,
      jsonb_build_object(
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (
      NEW.org_id,
      COALESCE(NEW.reporter_id, auth.uid()), -- ✅ FIXED: Use reporter_id if set, fallback to auth.uid()
      'task_created',
      'task',
      NEW.id,
      jsonb_build_object(
