-- Update trigger function to include functie_niveau, werkvorm, regio for personalized emails
  ...
  IF existing_goal_count = 0 THEN
    INSERT INTO public.agent_goals (
      ...
      jsonb_build_object(
        ...
        'regio', NEW.extracted_data->>'regio',
        'email_subject', NEW.email_subject
      )
    );
    
    RAISE LOG 'Created send_welcome_and_intake goal for application % (completeness: %, functie: %)', 
      NEW.id, 
      COALESCE(NEW.completeness_score, 0),
      COALESCE(NEW.extracted_data->>'functie_niveau', 'onbekend');
  ELSE
