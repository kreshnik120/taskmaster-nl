-- Fix VOG trigger: 'pending' is geen geldige status, moet 'validating' zijn
...
BEGIN
  ...
  IF new_vog_path != '' AND new_vog_path != old_vog_path THEN
    -- Set status to validating (NOT 'pending' - that's not a valid status!)
    NEW.vog_validation_status := 'validating';
