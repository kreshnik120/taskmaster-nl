-- Week 1-2: Data & PII Foundations Migration
-- Add new metadata columns to ai_knowledge_base for enhanced filtering and security

-- Create enum for confidentiality levels
CREATE TYPE confidentiality_level AS ENUM ('intern', 'vertrouwelijk');

-- Add new columns to ai_knowledge_base
ALTER TABLE public.ai_knowledge_base
ADD COLUMN valid_from DATE DEFAULT CURRENT_DATE,
ADD COLUMN valid_to DATE,
ADD COLUMN jurisdiction TEXT DEFAULT 'NL',
ADD COLUMN confidentiality confidentiality_level DEFAULT 'intern',
ADD COLUMN acl JSONB DEFAULT '[]'::jsonb,
ADD COLUMN role_tags TEXT[] DEFAULT ARRAY[]::TEXT[],
...
-- Create index for date range queries (validity window)
CREATE INDEX idx_knowledge_validity ON public.ai_knowledge_base(valid_from, valid_to) 
WHERE deleted_at IS NULL;

-- Create index for role-based filtering
CREATE INDEX idx_knowledge_role_tags ON public.ai_knowledge_base USING GIN(role_tags)
WHERE deleted_at IS NULL;

-- Create index for ACL filtering
CREATE INDEX idx_knowledge_acl ON public.ai_knowledge_base USING GIN(acl)
WHERE deleted_at IS NULL;

-- Create index for jurisdiction
CREATE INDEX idx_knowledge_jurisdiction ON public.ai_knowledge_base(jurisdiction)
WHERE deleted_at IS NULL;
...
CREATE TABLE IF NOT EXISTS public.pii_patterns (
  ...
  pattern_type TEXT NOT NULL, -- 'bsn', 'email', 'phone', 'postcode', 'iban'
...
-- Insert default PII patterns for Dutch context
INSERT INTO public.pii_patterns (pattern_type, regex_pattern, replacement_template) VALUES
('bsn', '\b\d{9}\b', '[BSN]'),
